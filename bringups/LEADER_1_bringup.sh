...
tmux send-keys "ros2 launch arduagent rover_bringup.launch.py ns:=leader1 is_leader:=True mqtt_params_file:=src/tuper/arduagent/config/mqtt_params_leader1.yaml" C-m

tmux select-pane -t 1
tmux send-keys "cd ~ && . start_tuper_container.sh" C-m
tmux send-keys "ros2 launch tuper_btcpp leader_bringup.launch.py ns:=leader1 tree_name:=TestRover" C-m
